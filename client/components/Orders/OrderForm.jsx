...
export default createForm('order', class OrderForm extends Component {
  ...
  addPackage(lineItemPackage) {
    this.props.initialValues.lineItems.map((lineItem, index) => {
  ...
  renderPackageButtons(packages) {
    return <div>
      {_.map(packages, (aPackage, index) => <Button key={index} onClick={this.addPackage.bind(this, aPackage)}>{aPackage.name}</Button>)}
    </div>
  }
